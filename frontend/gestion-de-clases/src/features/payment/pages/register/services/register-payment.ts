...
const registerPayment = async (payment: Payment) : Promise<Payment> => {
    try {
        const newStudent = await api.post<PaymentRequest>(URL_RENEW_PAYMENT, adapteRegistePaymentRequest(payment));
        return adapteRegisterPaymentRespone(newStudent.data);
    }
    catch(error: unknown) {
